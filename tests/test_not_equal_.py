...
def test_accuracy_not_equal_(shape, dtype):
    ...
    ref_out = ref_inp1.not_equal_(ref_inp2)
...
@pytest.mark.parametrize("shape", utils.POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_accuracy_not_equal_scalar_(shape, dtype):
    inp = torch.randint(0, 10, shape, dtype=dtype, device=flag_gems.device)
    scalar = 0

    ref_inp = utils.to_reference(inp.clone(), True)
    ref_out = ref_inp.not_equal_(scalar)
